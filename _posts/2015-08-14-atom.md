...
Si le problème persiste, c'est qu'un package n'est pas dimensionné pour la taille de votre projet. Dans ce cas, relancez Atom avec le safe mode via la commande ```atom --safe```. Si le problème persiste lors du démarrage de l'application, supprimez le contenu du fichier storage dans le répertoire storage (Settings > Open Config Folder).   
