...
vue file are compatibles. And this scenario happens for every major change we want to do. I tried
...
good alternative for these problems. Because we could gradually update our dependencies. But nothing
in life comes free. The biggest downside I see is the potential inconsistencies in the app. If the
payment part of the app is using a component in version 1 and another page is using the same
component in version 2, it can hurt the user experience. And the performances too because the same
library could potentially be loaded multiple time. At the moment, the granularity of our modules
makes the microfrontends transition too complicated as we have too many modules (~30).

## Managing state (Vuex)

![Vuex](./vuex.png)

Vuex provides a handy way to manage the state between components. At the beginning I really liked it
...
Later, another person wanted to introduce TypeScript. After many debates, we ended up with this
...
- **Unit tests** (with Jest): Less than 10% of our `xxx.test.js` are unit tests. It's mostly used to
  test business part of the app. If I have a complex function, I usually have tests for this one.
  Because writting integration test would require too many scenarios and make my test too verbose
  (integration tests are also slower than unit tests). Also, we don't unit test component or Vuex
  stores.
- **Integration tests** (with Vue Testing Library): where the magic happens. We really like this
  library, because it provides a very straightforward way to test our app.
